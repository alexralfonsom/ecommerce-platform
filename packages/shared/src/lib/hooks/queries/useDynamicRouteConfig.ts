...
} from '@lib/utils/routeConfigBuilder';
...
export function useDynamicRouteConfig(config: DynamicRouteConfigHookConfig = {}) {
  ...
  const superRouteConfig = React.useMemo(() => {
    ...
    const individualConfigs = menuQueries
      .map((menuQuery, index) => {
        ...
        if (!menuQuery.isSuccess || menuQuery.items.length === 0) {
          if (enableLogging) {
            console.log(
              `⚠️ ${menuTypeCode}: No items available (loading: ${menuQuery.isLoading}, error: ${!!menuQuery.error})`,
            );
          }
          return null;
        }
        ...
        if (enableLogging) {
          console.log(
            `✅ ${menuTypeCode}: Generated ${Object.keys(routeConfig).length} route configs`,
          );
        }
  ...
  const getRoutesByMenuType = React.useCallback(
    (menuTypeCode: string) => {
  ...
  const invalidateMenus = React.useCallback(
    (menuTypesToInvalidate?: string[]) => {
